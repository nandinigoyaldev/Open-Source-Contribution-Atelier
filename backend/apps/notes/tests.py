...
class NoteAPITestCase(TestCase):
    ...
    def test_notes_are_isolated_between_users(self):
        Note.objects.create(user=self.user1, title='User 1 Note', encrypted_content='A', iv='1')
        Note.objects.create(user=self.user2, title='User 2 Note', encrypted_content='B', iv='2')
    ...
    def test_update_note(self):
        self.client.force_authenticate(user=self.user1)
        note = Note.objects.create(user=self.user1, title='Old Title', encrypted_content='Old', iv='OldIV')
    ...
    def test_delete_note(self):
        self.client.force_authenticate(user=self.user1)
        note = Note.objects.create(user=self.user1, title='Del', encrypted_content='A', iv='1')
