...
from apps.sandbox.models import SnippetCollection, CodeSnippet
import json
...
def test_snippet_collection_crud(api_client, user, other_user):
    ...
    # Create Collection
    res = api_client.post("/api/sandbox/snippet-collections/", {"name": "Algorithms", "description": "Sorting"})
...
def test_snippet_isolation(api_client, user, other_user):
    ...
    res = api_client.patch(f"/api/sandbox/snippets/{snippet_id}/", {"title": "Hacked"}, format="json")
    assert res.status_code == 404
...
def test_snippet_cascade_set_null_on_collection_delete(api_client, user):
    ...
    # Create Collection
    res = api_client.post("/api/sandbox/snippet-collections/", {"name": "Temp Collection"})
    collection_id = res.data["id"]
    
    # Create Snippet inside collection
    res = api_client.post("/api/sandbox/snippets/", {
        "title": "Test Snip",
        "code": "pass",
...
def test_snippet_filters_combined(api_client, user):
    ...
    api_client.post("/api/sandbox/snippets/", {"title": "Bubble Sort", "code": "def", "collection": col1_id, "is_favorite": True}, format="json")
    api_client.post("/api/sandbox/snippets/", {"title": "Merge Sort", "code": "def", "collection": col1_id, "is_favorite": False}, format="json")
    api_client.post("/api/sandbox/snippets/", {"title": "Quick Sort", "code": "def", "collection": col2_id, "is_favorite": True}, format="json")
    
    # Combine collection and is_favorite
    res = api_client.get(f"/api/sandbox/snippets/?collection={col1_id}&is_favorite=true")
    assert len(res.data) == 1
...
def test_snippet_creation_missing_required_fields(api_client, user):
    ...
    res = api_client.post("/api/sandbox/snippets/", {"title": "No Code Snippet"}, format="json")
    assert res.status_code == 400
