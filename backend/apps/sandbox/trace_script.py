...
def safe_serialize(obj, max_depth=2, current_depth=0):
    ...
    if isinstance(obj, dict):
        return {str(k): safe_serialize(v, max_depth, current_depth + 1) for k, v in list(obj.items())[:50]}
    if isinstance(obj, set):
        return [safe_serialize(item, max_depth, current_depth + 1) for item in list(obj)[:50]]
    if isinstance(obj, tuple):
        return tuple(safe_serialize(item, max_depth, current_depth + 1) for item in obj[:50])
...
def run_trace(code_str):
    ...
    try:
        ...
        events.append({
            "step": len(events),
            "line": sys.exc_info()[2].tb_next.tb_lineno if sys.exc_info()[2] and sys.exc_info()[2].tb_next else 0,
            "event": "error",
            "locals": {k: safe_serialize(v) for k, v in namespace.items() if not k.startswith("__")},
            "stdout": get_current_stdout(),
