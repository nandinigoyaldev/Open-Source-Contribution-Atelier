...
class OrganizationType(DjangoObjectType):
    class Meta:
        model = Organization
        fields = ("id", "name", "slug", "description", "logo_url", "popularity_score", "date_added")

class ExerciseType(DjangoObjectType):
    class Meta:
        model = Exercise
        fields = ("id", "lesson", "title", "prompt", "expected_command", "explanation", "points")

class LessonType(DjangoObjectType):
    class Meta:
        model = Lesson
        fields = (
            "id", "organization", "difficulty", "title", "slug", "summary",
            "content", "learning_objectives", "tips", "category",
...
class Query(graphene.ObjectType):
    ...
    def resolve_all_lessons(root, info):
        ...
        if info.context.user.is_authenticated and hasattr(info.context.user, 'organization'):
