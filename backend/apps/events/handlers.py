...
def handle_quiz_completed(event: DomainEvent):
    ...
    logger.info(f"Processing QuizCompleted for user {user_id}, quiz {quiz_id}, score {score}")
    ...
    if passed:
        ...
        for badge in badges:
            ...
            if created:
                ...
                EventFactory.create_badge_unlocked(user, badge, {
...
def handle_certificate_generated(event: DomainEvent):
    ...
    logger.info(f"Processing CertificateGenerated for user {user_id}, certificate {certificate_name}")
