...
from apps.webhooks.security import compute_signature, verify_signature, require_webhook_signature
...
def test_decorator_custom_header(rf):
    decorated_view = require_webhook_signature(SECRET, header_name="HTTP_X_CUSTOM_SIG")(dummy_view)
    request = rf.post(
        "/webhook/",
        data=PAYLOAD,
        content_type="application/json",
