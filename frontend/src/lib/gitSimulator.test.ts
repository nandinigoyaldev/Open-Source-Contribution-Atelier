...
describe('gitSimulator', () => {
  ...
  it('should handle "git commit -m <msg>"', () => {
    ...
    expect(result.newState.branches[0].target).toBe(result.newState.commits[1].id);
  });

  it('should handle "git merge <branch>" without conflicts', () => {
    let current = state;
    current = parseGitCommand('git checkout -b feature', current).newState;
    current = parseGitCommand('git commit -m "feature commit"', current).newState;
  ...
  it('should handle "git merge <branch>" with conflicts', () => {
    let current = state;
    current = parseGitCommand('git checkout -b conflict-test', current).newState;
    current = parseGitCommand('git checkout main', current).newState;
  ...
  it('should block commands when in conflict state', () => {
    ...
    const blockedRes = parseGitCommand('git checkout feature', mergeRes.newState);
    expect(blockedRes.error).toBe("You must resolve conflicts before executing other commands.");
  });
