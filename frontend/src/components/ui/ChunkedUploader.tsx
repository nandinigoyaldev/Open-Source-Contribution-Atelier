...
import { CheckCircle, Loader2, Pause, Play, ShieldAlert, Upload, X } from "lucide-react";
import { fetchApi } from "../../lib/api";
import { getAccessToken } from "../../lib/authToken";

const CHUNK_SIZE = 5 * 1024 * 1024;
const TERMINAL_SCAN_STATUSES = new Set(["clean", "released", "infected", "rejected", "failed"]);
...
export function ChunkedUploader() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const response = (await fetchApi(`/uploads/status/${sessionId}/`)) as ScanStatusResponse;
        setStatus(response.status);
        setStatusMessage(response.message ?? "");
        if (TERMINAL_SCAN_STATUSES.has(response.status)) window.clearInterval(timer);
      } catch {
        setStatus("error");
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      if (!currentSessionId) {
        ...
        const response = (await fetchApi(`/uploads/status/${currentSessionId}/`)) as ScanStatusResponse;
  ...
  const blocked = status === "infected" || status === "rejected" || status === "failed";
      ...
      {!file ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="truncate pr-4 font-bold">{file.name} ({(file.size / 1048576).toFixed(2)} MB)</div>
            {!successful && <button onClick={cancelUpload} className="text-red-500 p-1" aria-label="Cancel upload"><X size={20} /></button>}
          </div>
          <div className="w-full bg-gray-200 rounded-full h-4 dark:bg-gray-800 border-2 border-black dark:border-[#2e2924]">
            <div className={`h-full rounded-full transition-all ${blocked || status === "error" ? "bg-red-500" : "bg-primary"}`} style={{ width: `${progress}%` }} />
          </div>
          <div className="flex items-center justify-between gap-3 font-bold text-sm">
            <span className="text-muted">
              ...
              {(blocked || status === "error") && (statusMessage || "Upload blocked")}
            </span>
            ...
            {["idle", "paused", "error"].includes(status) && (
              <button onClick={startUpload} className="bg-black text-white px-3 py-1 rounded-full text-xs flex items-center gap-1 dark:bg-white dark:text-black">
                <Play size={14} /> {status === "idle" ? "Start" : "Resume"}
              </button>
            )}
            {status === "uploading" && (
              <button onClick={() => { uploadRef.current.abort = true; }} className="bg-black text-white px-3 py-1 rounded-full text-xs flex items-center gap-1">
                <Pause size={14} /> Pause
              </button>
            )}
          </div>
