...
export function SnippetLibraryModal({ isOpen, onClose, onInsertCode }: SnippetLibraryModalProps) {
  const {
    collections, snippets, isLoading,
  ...
  const [newSnippetData, setNewSnippetData] = useState({ title: '', code: '', language: 'python', description: '', tags: '' });
  ...
  const handleCreateSnippet = async (e: React.FormEvent) => {
    ...
    setNewSnippetData({ title: '', code: '', language: 'python', description: '', tags: '' });
      ...
      <div className="bg-white dark:bg-[#151411] w-full max-w-6xl h-[85vh] rounded-2xl border-4 border-black dark:border-[#2e2924] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b-4 border-black dark:border-[#2e2924] bg-white dark:bg-[#1f1c18]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-gray-900 dark:text-white">Snippet Library</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Save and reuse your code</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-2 ml-2 hover:bg-gray-100 dark:hover:bg-[#2e2924] rounded-lg transition-colors">
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 border-r-4 border-black dark:border-[#2e2924] bg-gray-50 dark:bg-[#1a1816] flex flex-col">
            <div className="p-4 space-y-2 flex-1 overflow-y-auto">
              ...
              <button
                onClick={() => { setActiveCollectionId(null); setShowFavoritesOnly(false); }}
              ...
              <button
                onClick={() => { setActiveCollectionId(null); setShowFavoritesOnly(true); }}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl font-bold text-sm transition-all border-2 ${
                  showFavoritesOnly 
              ...
              <div className="pt-6 pb-2">
                <div className="flex items-center justify-between px-2 mb-2">
                  <span className="text-xs font-black text-gray-400 uppercase tracking-wider">Collections</span>
                  <button onClick={() => setIsCreatingCollection(true)} className="text-gray-400 hover:text-black dark:hover:text-white transition-colors">
                ...
                {collections.map(c => (
                  <div key={c.id} className="group flex items-center gap-2">
                    <button
                      onClick={() => { setActiveCollectionId(c.id); setShowFavoritesOnly(false); }}
                    ...
                    <button onClick={() => removeCollection(c.id)} className="opacity-0 group-hover:opacity-100 p-1 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-[#151411]">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-[#1a1816]">
              {isCreatingSnippet && (
                <form onSubmit={handleCreateSnippet} className="mb-6 bg-white dark:bg-[#1f1c18] border-2 border-black dark:border-[#2e2924] rounded-2xl p-4 shadow-sm animate-in slide-in-from-top-4">
                  <h3 className="font-black text-lg mb-4">Create New Snippet</h3>
                  <div className="space-y-4">
                    <div className="flex gap-4">
                      <input 
                        ...
                        onChange={e => setNewSnippetData(p => ({...p, title: e.target.value}))}
                      />
                      <select 
                        ...
                        onChange={e => setNewSnippetData(p => ({...p, language: e.target.value}))}
                      >
                    ...
                    <textarea 
                      ...
                      onChange={e => setNewSnippetData(p => ({...p, code: e.target.value}))}
                    />
                    <input 
                      type="text" 
                      ...
                      onChange={e => setNewSnippetData(p => ({...p, tags: e.target.value}))}
                    />
                    <div className="flex justify-end gap-2 pt-2">
                      <button type="button" onClick={() => setIsCreatingSnippet(false)} className="px-4 py-2 font-bold hover:bg-gray-100 dark:hover:bg-[#2e2924] rounded-xl transition-colors">Cancel</button>
                      <button type="submit" className="px-6 py-2 bg-black text-white dark:bg-white dark:text-black font-bold rounded-xl hover:brightness-110 transition-all border-2 border-black">Save Snippet</button>
                    </div>
                  </div>
                </form>
              )}
              ...
              {isLoading ? (
                <div className="flex justify-center items-center h-full text-gray-500">Loading snippets...</div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
                  ...
                  <button onClick={() => setIsCreatingSnippet(true)} className="text-blue-500 hover:underline">Create your first snippet</button>
                </div>
              ) : (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {snippets.map(snippet => (
                    <div key={snippet.id} className="bg-white dark:bg-[#1f1c18] border-2 border-black dark:border-[#2e2924] rounded-2xl overflow-hidden hover:-translate-y-1 hover:shadow-lg transition-all duration-200 group">
                      <div className="p-4 border-b-2 border-gray-100 dark:border-[#2e2924] flex items-start justify-between gap-4">
                        <div>
                          <h3 className="font-black text-lg text-gray-900 dark:text-white mb-1">{snippet.title}</h3>
                          <div className="flex flex-wrap gap-2">
                            <span className="px-2 py-0.5 bg-gray-100 dark:bg-[#2e2924] text-xs font-bold rounded-md uppercase tracking-wider">{snippet.language}</span>
                            {snippet.tags.map(t => (
                              <span key={t} className="px-2 py-0.5 bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 text-xs font-bold rounded-md">#{t}</span>
                            ))}
                          </div>
                        </div>
                        <div className="flex items-center gap-1">
                          <button onClick={() => toggleFavorite(snippet)} className={`p-2 rounded-lg transition-colors ${snippet.is_favorite ? 'text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-900/20' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-[#2e2924]'}`}>
                            <Star className="w-5 h-5" fill={snippet.is_favorite ? "currentColor" : "none"} />
                          </button>
                          <button onClick={() => removeSnippet(snippet.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                      ...
                      <div className="bg-[#1e1e1e] p-4 text-sm font-mono text-gray-300 overflow-x-auto relative group/code">
                        ...
                        <div className="absolute top-2 right-2 flex items-center gap-2 opacity-0 group-hover/code:opacity-100 transition-opacity">
                          <button onClick={() => handleCopy(snippet.code)} className="p-2 bg-gray-800 text-white rounded-lg hover:bg-gray-700 transition-colors shadow-lg" title="Copy to Clipboard">
                            <Copy className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleInsert(snippet.code)} className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors shadow-lg flex items-center gap-2 font-bold" title="Insert into Editor">
