...
test.describe("Authentication Flows", () => {
  test("User can navigate to login and see appropriate fields", async ({ page }) => {
    await page.goto("/login");
    await expect(page).toHaveTitle(/Login/i).catch(() => {}); // Optional, depending on actual title
    await expect(page.getByPlaceholder("the_smartest@kid.com")).toBeVisible();
    await expect(page.getByPlaceholder("••••••••")).toBeVisible();
    await expect(page.getByRole("button", { name: "Let Me In!" })).toBeVisible();
  ...
  test("User sees error on failed login", async ({ page }) => {
    ...
    await expect(page.locator("text=Failed to login").or(page.locator("text=Invalid credentials"))).toBeVisible();
  });

  test("User can navigate to signup and see appropriate fields", async ({ page }) => {
